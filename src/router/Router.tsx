  ...
  return (
     ...
     { token ?  <Route path='/' element={role.toLowerCase() === "admin" ?
      <Admin/> : <Home /> } />  :  <Route path="/" element={<SignIn />} /> }
        ...
        <Route path="/editPoll/:edittitleId" element={<EditPoll />} />
        <Route path="/AddOptions/:optionId" element={<AddOptions />} />

          <Route path="/admin" element={<PrivateRoute login={(localStorage.getItem("token") && localStorage.getItem("role") === "admin")}>  <Admin /></PrivateRoute>}>
      </Route>
      <Route path="/home" element={<PrivateRoute login={(localStorage.getItem("token") && localStorage.getItem("role") === "guest")}>  <Home /></PrivateRoute>}>
      </Route>
